...
e.test = function(tc, endpoint, request, response, _delimiters) {
    ...
    if (response && (response.body || response.bodyFile)) {
        ...
        if (response.body) expectedResponse = parseData(JSON.stringify(response.body));
        else if (response.bodyFile) expectedResponse = parseData(cli.readFile("lib/" + response.bodyFile));
